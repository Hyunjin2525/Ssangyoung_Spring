		...
		<div class="row">
			<table class="table">
				<tr>
					...
					<td width=80%>
						<input type=text ref="id" size=15 class="input-sm" v-model="id"> <!-- v-model input 시 자동으로 데이터를 넣어줌 -->
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td width=80%>
						<input type=radio value="남자" ref="sex" name="sex" checked v-model="sex">남자
		...
		new Vue({
			...
			methods:{
				join:function(){
					...
					//alert(this.sex)
					axios.post('http://localhost/web/member/join_ok.do',null,{ //null = config 자리 (파일업로드시에만 사용)
						params:{
							id:this.id,
							pwd:this.pwd,
							name:this.name,
