...
import com.sist.vo.*;
import com.sist.dao.*;
//Security 5==> 반드시 BCryptPasswordEncoder를 추가해야 됨
@RestController
public class MemberRestController {
	...
	public String member_join(MemberVO vo)
	{
		String result="";
		try {
			String en=encoder.encode(vo.getPwd()); //비밀번호 암호화
			vo.setPwd(en);
	...
	public String member_login_ok(String id, String pwd,HttpSession session)
	{
		...
		else 
		{
			...
			if(encoder.matches(pwd, vo.getPwd())) //비밀번호 동일한지 체크(복구화)(암호화 된 데이터랑 실제 값 비교 방법)
			{
				//로그인
				result="OK";
				//세션 저장
