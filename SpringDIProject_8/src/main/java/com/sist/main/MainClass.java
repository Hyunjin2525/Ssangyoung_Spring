 ...
 *		=> 스프링의 목료(로드 존스) = Open Source
 *			1) 클래스와 클래스간의 느슨한 결합, 낮은 결합도 유지
 *							   ----------------------
 *								변경할 때 추가될 때 클래스에 영향이 없이 개발
 *								POJO (독립적인 클래스 제작)
 ...
 *		*** 스프링에서 지원하는 라이브러리
 *			 Spring Core : DI (객체 생성 - 소명) => Container
 ...
 *						@ControllerAdvice : 공통 예외차리
 *						@RestControllerAdvice : 공통 예외처리
 *						------------------------ 클래스에 설정을 하면 컨테이너가 메모리를 할당한다
 *				*** 빈(객체) 생명주기
 *					빈 생성 = DI = init_method = 객체사용(개발자) = distroy-method = 소멸
 *				자바코드
 *			 Spring MVC
 *		3. AOP의 개념
 *		4. MVC구조 파악
...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		String path="C:\\springDev\\springStudy\\SpringDIProject_8\\src\\main\\java\\com\\sist\\main\\ClassPathApplicationContext.java";
		AppplicationContext app=new ClassPathApplicationContext(path);
