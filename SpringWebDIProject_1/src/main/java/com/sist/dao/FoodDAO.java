 ...
 * 					= HandlerAdapter : Model를 찾는 역할   					=> web.xml에 셋팅과 동시에 구동
...
public class FoodDAO {
	...
	//@Select("SELECT cno,title,poster,subject FROM food_category ORDER BY cno ASC")
	public List<CategoryVO> foodCategoryListData()
	{
		return mapper.foodCategoryListData();
	}
	
	//@Select("SELECT fno,name,score,address,phone,type,poster FROM food_house WHERE cno=#{cno}")
