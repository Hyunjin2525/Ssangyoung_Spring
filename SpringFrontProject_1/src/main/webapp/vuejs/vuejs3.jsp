 		...
 		<table class="table">
 			...
 			<tr v-for="vo in movie">	
 				...
 				<td class="text-center">
 					<img :src="'https://www.kobis.or.kr'+vo.thumbUrl" style="width: 30px;height: 30px"> <!--  속성안에 값을 가져올때는 :를 쓴다 -->
 				</td>
 				<td>{{vo.movieNm}}</td>
 ...
 <script>
 	new Vue({
 		...
 		beforeUpdate:function(){
 			console.log("beforeUpdate:상태가 변경(데이터값 변경)되시 직전상태...")
 			//멤버 변수 설정 : react=>state, Vue => data = data를 다른 메소드에서 변경시에는 this
 		},
 		/**/updated:function(){
 			console.log("updated: 데이터 변경이나 수정이 완료된 상태 => mounted 재호출...")
 		},
