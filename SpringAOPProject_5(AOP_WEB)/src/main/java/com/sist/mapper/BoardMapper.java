...
public interface BoardMapper {
	...
	@Select("SELECT no,subject,name,TO_CHAR(regdate, 'YYYY-MM-DD') as dbday,hit,group_tab,num "
			...
			+ "FROM(SELECT no,subject,name,regdate,group_tab,hit FROM springReplyBoard ORDER BY group_id DESC,group_step ASC)) "
	...
	//추가
	@Insert("INSERT INTO springReplyBoard(no,name,subject,content,pwd,group_id) VALUES(srb_no_seq.nextval,"
			+ "#{name},#{subject},#{content},#{pwd},(SELECT NVL(MAX(group_id)+1,1) FROM springReplyBoard))")
	public void boardInsert(BoardVO vo);
	//상세보기
	@Update("UPDATE springReplyBoard SET hit=hit+1 WHERE no=#{no}")
	public void hitIncrement(int no);
	@Select("SELECT no,name,subject,content,TO_CHAR(regdate, 'YYYY-MM-DD') as dbday,hit FROM springReplyBoard WHERE no=#{no}")
	...
	@Insert("INSERT INTO springReplyBoard(no,name,subject,content,pwd,group_id,group_step,group_tab,root) VALUES(srb_no_seq.nextval,"
			+ "#{name},#{subject},#{content},#{pwd},#{group_id},#{group_step},#{group_tab},#{root})")
	...
	//삭제 ===== 트랜잭션 (스프링 : AOP)
	@Select("SELECT root,depth FROM springReplyBoard WHERE no=#{no}")
	public BoardVO boardInfoData(int no);
	@Update("UPDATE springReplyBoard SET subject='관리자가 삭제한 게시물입니다',content='관리자가 삭제한 게시물입니다' WHERE no=#{no}")
	...
	@Select({
		...
		+ "subject LIKE '%'||#{ss}||'%'"
		+ "</when>"
		+ "<when test=\"fd=='C'.toString()\">" //숫자로 들어오기 때문에 toString
		+ "content LIKE '%'||#{ss}||'%'"
		+ "</when>"
		+ "</choose>"
		+ "</trim>"
