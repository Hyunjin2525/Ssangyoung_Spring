...
public interface FoodMapper {
	//동적 쿼리를 이용하는 경우
	@Select("<script>"
			+"SELECT cno,poster,title FROM food_category WHERE"
			+"<if test=\"cno==1\">"
			...
			+"ORDER BY cno ASC"
			+"</script>")
	public List<CategoryVO> foodCategoryData(Map map);//동적쿼리는 map만 인식함
	...
	@Select("SELECT fno,cno,name,phone,type,score,address,poster "
