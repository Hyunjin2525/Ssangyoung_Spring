 ...
 *  						-----
 *  						필요한 데이터가 있는 경우에 데이터를 전송 (맴버변수의 초기화)
 *  						변수의 초기화
 *  						----------
 ...
 * 						=> PathValiable (admin/hong/1234)
 		*** 요청을 받는 경우 (요청 데이터 전송)
 			=> request안에 담겨서 돌아온다 => request.getParameter();
 		...
 		2) 메소드
 			...
 			= 매개변수 : JSP에서 제공하는 내장객체 설정
 						...
 						*** 순서는 상관없다
 		3) 찾기 (메소드)
 			@GetMapping => Get방식으로 요청 (default) => <a> ajax , axious...
 			@PostMapping => <form> , ajax , axios
 											----- ajax대체 => Vue,React
 							axios.get() , axious.post()
 			-------------------------- Spring 4.3이후로부터 나눠져 있다
 		...
 		=> MVC 구동 방식
 			...
 			3) DispatchertServlet => HandlerMapping
 								클래스를 찾아서 메소그 호출하라고 전송
